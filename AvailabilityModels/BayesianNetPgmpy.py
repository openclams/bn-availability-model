    ...
    def construct_bn(self):
        ...
        for node_name in self.G.V:
            ...
            if len(list(self.bn.get_parents(node_name))) > 0:
                ...
                self.bn.get_cpds(node_name).values[:,self.bn.get_cpds(node_name).values.shape[1]-1] = np.array([1-self.G.V[node_name].availability,self.G.V[node_name].availability])
            else:
                # If we have a root node
                self.bn.get_cpds(node_name).values = np.array([1 - self.G.V[node_name].availability,self.G.V[node_name].availability])
        ...
        for service in self.app["services"]:
            ...
            for idx, host in enumerate(service["servers"]):
                ...
                self.bn.add_edge(host, server_name)
            ...
            for path in all_paths:
                self.bn.add_node(path, 2)
                for component in all_paths[path]:
                    self.bn.add_edge(component, path)
            ...
            for endpoints in path_channel_map:
                ...
                self.bn.add_edge(path_channel_map[endpoints],channel_name)
           ...
           #computing external channel
                ...
                if len(paths) == 0:
                    ...
                    for c in all_paths:
                        if path == all_paths[c]:
                            ...
                            self.bn.add_edge( c , external_channels[i]) # I need here the channel name
                            found = True
                    if not found:
                        # We have a new path
                        path_name = service["name"] + "_P_" + str(current_path_index)
                        ...
                        self.bn.add_edge( path_name,external_channels[i])
            ...
            self.bn.add_node( service["name"], 2)
            self.bn.add_edge(K, service["name"])
            self.bn.add_edge(OR, service["name"])
